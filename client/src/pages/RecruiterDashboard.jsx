            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-blue-200 text-sm">
                  Discover top talent across campuses
                </p>
              </div>
            </div>
          </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-6 h-32 animate-pulse shadow-md"
              />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <StatCard
              ...
              value={stats?.savedCandidates ?? 0}
              subtitle="Shortlisted profiles"
              color="bg-gradient-to-br from-blue-500 to-blue-600"
            />
            <StatCard
              icon={Building2}
              title="Colleges in Pipeline"
              value={stats?.viewedColleges ?? 0}
              subtitle="Institutions explored"
              color="bg-gradient-to-br from-purple-500 to-purple-600"
            />
            <StatCard
              icon={Users}
              title="In Interviews"
              value={stats?.statusBreakdown?.interviewing ?? 0}
            ...
            <StatCard
              icon={Award}
              title="Hired"
              value={stats?.statusBreakdown?.hired ?? 0}
