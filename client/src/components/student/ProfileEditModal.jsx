...
const ProfileEditModal = ({ student, open, onClose, onProfileUpdate }) => {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      expectedSalaryMin:
        student.expectedSalaryMin != null
          ? String(student.expectedSalaryMin)
          : "",
      expectedSalaryMax:
        student.expectedSalaryMax != null
          ? String(student.expectedSalaryMax)
          : "",
  ...
  const addSkill = () => {
    ...
    setSkills((prev) => [
      ...prev,
      { skillName: name, proficiencyLevel: newSkill.proficiencyLevel },
    ]);
  ...
  const updateSkillLevel = (idx, level) =>
    setSkills((prev) =>
      prev.map((s, i) => (i === idx ? { ...s, proficiencyLevel: level } : s)),
    );
  ...
  const removeLocation = (idx) =>
    setPreferredLocations((prev) => prev.filter((_, i) => i !== idx));
  ...
  const validate = () => {
    ...
    if (
      form.graduationYear &&
      (form.graduationYear < 2000 || form.graduationYear > 2050)
    )
  ...
  const renderAcademic = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
      ...
      <Field label="Cleared Backlogs">
        <Input
          value={form.clearedBacklogs}
          onChange={set("clearedBacklogs")}
          type="number"
          min="0"
          placeholder="0"
        />
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <Input
            ...
            onChange={(e) =>
              setNewSkill((p) => ({ ...p, skillName: e.target.value }))
            }
            onKeyDown={(e) => e.key === "Enter" && addSkill()}
            placeholder="Add a skill (e.g. React, Python…)"
          />
          {errors.newSkill && (
            <p className="text-xs text-red-500 mt-1">{errors.newSkill}</p>
          )}
        </div>
        <select
          value={newSkill.proficiencyLevel}
          onChange={(e) =>
            setNewSkill((p) => ({ ...p, proficiencyLevel: e.target.value }))
          }
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        >
          ...
          {toast && (
            <div
              className={`flex items-center gap-2 px-5 py-3 text-sm font-medium flex-shrink-0 ${
                toast.type === "success"
