  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="mt-4 text-slate-500 font-medium">Loading your detailed feedback...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto animate-fadeIn space-y-6">
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6 border-b border-slate-100">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 mb-2">Interview Feedback</h2>
            <div className="flex items-center gap-2 text-sm text-slate-500">
               Role: <span className="font-semibold text-indigo-600">{interview.targetRole}</span>
               •
          ...
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 px-6 py-4 rounded-2xl border border-indigo-100 flex items-center gap-4">
            <div className={`text-4xl font-black ${interview.overallScore >= 80 ? 'text-green-600' : interview.overallScore >= 60 ? 'text-yellow-600' : 'text-red-500'}`}>
            ...
            <div key={index} className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div className={`px-3 py-1 rounded-lg text-sm font-bold border ${
                  response.score >= 8 ? 'bg-green-100 text-green-700 border-green-200' : 
                  response.score >= 5 ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 
              ...
              {response.recommendedSkill && (
                <div className="mt-4 pl-11">
                  <div className="bg-orange-50/80 p-4 rounded-xl border border-orange-200 flex items-start gap-4">
                    ...
                    <div>
                      <h5 className="text-orange-800 font-bold mb-1">Recommended Learning Need</h5>
                      <p className="text-orange-700 text-sm">
                        Based on your answer, the AI recommends reviewing <span className="font-bold underline">{response.recommendedSkill}</span> to improve your performance.
                      </p>
                    </div>
                  </div>
                </div>
