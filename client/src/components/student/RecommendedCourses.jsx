    ...
    return (
      <div className="bg-white p-12 text-center rounded-3xl border border-slate-100 shadow-sm">
        ...
        <p className="text-slate-500 font-medium">
          Run a skill gap analysis to get personalized curriculum
          recommendations.
        </p>
      </div>
    );
            ...
            {courses.map((course, idx) => (
              ...
              >
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-sm text-slate-500 font-medium">
                  ...
                  <span
                    className={`px-2 py-1 rounded-md text-xs font-bold ${course.price?.toLowerCase() === "free" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-700"}`}
                  >
                    {course.price || "Paid"}
                  </span>
                </div>
        ...
        {certs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {certs.map((cert, idx) => (
              <div
                key={idx}
                className="bg-gradient-to-br from-slate-50 to-white rounded-2xl border border-slate-200 p-6 flex justify-between items-center shadow-sm hover:shadow-md transition-all"
              >
                <div>
