    ...
    return (
      <div className="bg-white p-12 text-center rounded-3xl border border-slate-100 shadow-sm">
        ...
        <p className="text-slate-500 font-medium">
          Run an analysis to generate your skill radar
        </p>
      </div>
    );
  }
        ...
        <div className="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
          {analysis?.missingSkills?.length > 0 ? (
            analysis.missingSkills.map((skill, i) => (
              <div
                key={i}
                className="flex justify-between items-center p-4 rounded-2xl border border-slate-100 hover:border-indigo-100 hover:bg-slate-50 transition group"
              >
