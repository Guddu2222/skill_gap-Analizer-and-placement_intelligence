...
const DOMAINS = [
  ...
  },
  {
    ...
    desc: "ML Engineers, Data Analysts",
  },
  {
    id: "cloud-devops",
    name: "Cloud & DevOps",
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {DOMAINS.map(({ id, name, icon: Icon, gradient, desc }) => (
          ...
          >
            <Icon
              className={`w-7 h-7 mb-3 ${domain === id ? "text-white" : "text-gray-500"}`}
            />
            <p className="font-bold text-sm mb-1">{name}</p>
            <p
              className={`text-xs ${domain === id ? "text-white/80" : "text-gray-400"}`}
            >
              {desc}
            </p>
          </button>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {experts.map((e) => (
              <ExpertCard key={e._id} expert={e} onSave={handleSave} />
            ))}
          </div>
        ) : (
          <div className="bg-white rounded-2xl p-12 text-center shadow-md">
            <p className="text-gray-400 text-sm">
              No experts found for this domain yet. Add more student profiles to
              see results.
            </p>
          </div>
        )}
