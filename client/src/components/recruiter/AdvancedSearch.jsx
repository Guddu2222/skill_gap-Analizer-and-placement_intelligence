    ...
    <div className="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4 text-center">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">Grad Year</p>
        </div>
        <div>
          <p className="text-lg font-bold text-gray-900">
...
const AdvancedSearch = () => {
  ...
  const removeSkill = (s) =>
    setCriteria((c) => ({ ...c, skills: c.skills.filter((x) => x !== s) }));

  const toggleArr = (key, val) =>
    setCriteria((c) => ({
      ...c,
      [key]: c[key].includes(val)
        ? c[key].filter((x) => x !== val)
        : [...c[key], val],
    }));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-md">
        <h2 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
          <Sliders className="w-5 h-5 text-blue-600" /> Advanced Candidate
          Search
        </h2>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-1.5 mt-3">
            ...
            {POPULAR_SKILLS.filter((s) => !criteria.skills.includes(s)).map(
              (s) => (
                <button
                  key={s}
                  onClick={() => addSkill(s)}
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setCriteria((c) => ({ ...c, minCgpa: e.target.value }))
                }
                placeholder="Min"
              ...
              <input
                ...
                max="10"
                step="0.1"
                value={criteria.maxCgpa}
                onChange={(e) =>
                  setCriteria((c) => ({ ...c, maxCgpa: e.target.value }))
                }
                placeholder="Max"
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
              />
            ...
            <div className="space-y-1 max-h-28 overflow-y-auto">
              {DEPARTMENTS.map((d) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={criteria.departments.includes(d)}
                    onChange={() => toggleArr("departments", d)}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {GRAD_YEARS.map((yr) => (
                <button
                  key={yr}
                  onClick={() => toggleArr("graduationYears", yr)}
            ...
            <div className="flex flex-wrap gap-2">
              {["beginner", "intermediate", "advanced", "expert"].map((lvl) => (
                <button
                  key={lvl}
                  onClick={() => toggleArr("skillProficiency", lvl)}
        ...
        <div>
          <p className="text-sm font-semibold text-gray-700 mb-4">
            {results.length > 0
              ...
              : "No candidates match your criteria. Try widening your filters."}
          </p>
          {results.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {results.map((c) => (
                <CandidateCard key={c._id} candidate={c} onSave={handleSave} />
              ))}
            </div>
          )}
        </div>
