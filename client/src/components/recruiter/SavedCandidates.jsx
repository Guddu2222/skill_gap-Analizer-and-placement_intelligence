    ...
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="bg-white rounded-2xl p-6 shadow-md animate-pulse h-24"
          />
        ))}
      </div>
    );
          ...
          <div className="flex flex-wrap gap-2">
            {STATUS_OPTIONS.map((s) =>
              stats.statusBreakdown[s] ? (
                <span
                  key={s}
                  className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${STATUS_STYLES[s]}`}
                >
      ...
      {displayed.length === 0 ? (
        <div className="bg-white rounded-2xl p-16 text-center shadow-md">
          ...
          <p className="text-sm text-gray-400">
            Use Advanced Search or Domain Experts to discover and save
            candidates.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {displayed.map((saved) => {
            ...
            return (
              <div
                key={saved._id}
                className="bg-white rounded-2xl p-5 shadow-md border border-gray-100 flex items-center gap-5"
              >
                {/* Avatar */}
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  <select
                    value={saved.status}
                    onChange={(e) =>
                      handleStatusChange(saved._id, e.target.value)
                    }
