      ...
      >
        ...
        {loading ? (
          ...
          <>
              ...
              {[
                {
                  ...
                  value: detail.statistics?.totalStudents,
                },
                {
                  label: "Available Now",
                  value: detail.statistics?.availableStudents,
                  color: "text-green-600",
                },
                { label: "Placed", value: detail.statistics?.placedStudents },
                {
                  label: "Placement Rate",
                  value: `${detail.statistics?.placementRate}%`,
                  color: "text-blue-600",
                },
              ].map(({ label, value, color }) => (
                <div key={label} className="bg-gray-50 rounded-xl p-4">
                  <p
                    className={`text-2xl font-bold ${color || "text-gray-900"}`}
                  >
                    {value ?? "–"}
                  </p>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {detail.topSkills.map((skill) => (
                    ...
                    >
                      {skill.name}{" "}
                      <span className="text-blue-400">({skill.count})</span>
                    </span>
                  ))}
                </div>
              </div>
            ...
            {/* Departments */}
            {detail.departmentStats &&
              Object.keys(detail.departmentStats).length > 0 && (
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">
                    Department Breakdown
                  </h3>
                  <div className="space-y-2">
                    {Object.entries(detail.departmentStats).map(
                      ([dept, stats]) => (
                        <div
                          key={dept}
...
const CollegeCard = ({ college, onSelect }) => (
  ...
  >
    <div className="flex items-start justify-between mb-4">
      ...
      <span
        className={`px-2.5 py-1 rounded-full text-xs font-semibold border flex-shrink-0 ${getTierStyle(college.tier)}`}
      >
    ...
    <div className="grid grid-cols-2 gap-3 mb-4">
      <div className="bg-blue-50 rounded-xl p-3">
        <p className="text-xl font-bold text-gray-900">
          {college.metrics.totalStudents}
        </p>
        <p className="text-xs text-gray-500">Total Students</p>
      </div>
      <div className="bg-green-50 rounded-xl p-3">
        <p className="text-xl font-bold text-green-700">
          {college.metrics.placementRate}%
        </p>
    ...
    <div className="flex items-center justify-between text-xs text-gray-500 mb-4 px-1">
      <span>
        ...
        <strong className="text-gray-800">
          ₹{college.metrics.avgPackage}L
        </strong>
      </span>
      <span>
        Highest:{" "}
        <strong className="text-gray-800">
          ₹{college.metrics.highestPackage}L
        </strong>
      </span>
      <span>
        Available:{" "}
        <strong className="text-green-600">
          {college.metrics.availableStudents}
        </strong>
      </span>
    </div>
...
const CollegeExplorer = () => {
  ...
  const loadColleges = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchRecruiterColleges({
        ...filters,
        page: 1,
        limit: 12,
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-md">
        ...
        {showFilters && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5 pt-5 border-t border-gray-100">
            ...
            <input
              ...
              onChange={(e) =>
                setFilters((f) => ({ ...f, minPlacementRate: e.target.value }))
              }
            ...
            <input
              ...
              onChange={(e) =>
                setFilters((f) => ({ ...f, minStudents: e.target.value }))
              }
        ...
        <p className="text-xs text-gray-400 mt-4">
          Showing{" "}
          <span className="font-semibold text-gray-700">{colleges.length}</span>{" "}
          of{" "}
