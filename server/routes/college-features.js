...
const getSkillName = (s) =>
  (typeof s === "string" ? s : s && s.skillName) || "";
...
async function resolveCollegeIds(collegeId) {
  ...
  try {
    ...
    const matches = await College.find({ name: nameRegex })
      .select("_id")
      .lean();
    matches.forEach((m) => {
  ...
  async (req, res) => {
    try {
      ...
      if (!collegeId)
        return res
          .status(403)
          .json({ error: "College not associated with this account" });
      ...
      const placementRate =
        totalStudents > 0
          ...
          : 0;
      const withPackage = students.filter(
        (s) =>
          s.placementPackage != null &&
      ...
      const avgPackage =
        withPackage.length > 0
          ? (
              withPackage.reduce(
                (sum, s) => sum + (s.placementPackage || 0),
                0,
              ) / withPackage.length
            ).toFixed(2)
          : 0;
      ...
      students.forEach((student) => {
        ...
        if (student.isPlaced || student.placementStatus === "placed")
...
router.get(
  ...
  async (req, res) => {
    try {
      const collegeId = req.user.collegeId;
      if (!collegeId)
        return res.status(403).json({ error: "College not associated" });
      ...
      if (graduationYear)
        query.graduationYear =
          parseInt(graduationYear, 10) || query.graduationYear;
    ...
    try {
      ...
      students.forEach((student) => {
        (student.skills || []).forEach((skill) => {
          ...
          if (skillsMap[name].proficiencyLevels[level] !== undefined)
            skillsMap[name].proficiencyLevels[level]++;
      ...
      const topSkills = Object.values(skillsMap)
        ...
        .slice(0, 20);
      const skillsByCategory = Object.entries(categoryMap).map(
        ([category, count]) => ({
          category,
          count,
    ...
    try {
      ...
      students.forEach((student) => {
        (student.skills || []).forEach((skill) => {
          ...
          if (!name) return;
          if (!skillsMap[name])
            skillsMap[name] = { name, count: 0, students: [] };
          skillsMap[name].count++;
...
router.get("/curriculum-gap", auth, async (req, res) => {
  try {
    ...
    const studentQuery = req.user.collegeId
    ...
    const gapAnalysis = Object.keys(marketDemand)
      .map((skill) => ({
        skill,
    ...
    const criticalGaps = gapAnalysis.filter(
      (g) => g.gap > 0 && g.supplyCount < g.demandCount * 0.5,
    );
  ...
  try {
    ...
    const atRiskList = students
      .map((student) => {
        ...
        const riskReason = [];
        if (student.cgpa != null && student.cgpa < 7)
          riskReason.push("Low CGPA");
...
router.post("/shortlist", auth, async (req, res) => {
  try {
    ...
    if (requiredSkills && requiredSkills.length > 0) {
      query.$and = requiredSkills.map((skill) => ({
        "skills.skillName": skill,
      }));
    }

    const students = await Student.find(query).select(
      "rollNumber cgpa skills department placementStatus firstName lastName",
    );
