...
function doActionsOnSuccess(msg) {
    ...
    Config.getAll().then(config => {
        ...
        if (callback) {
            ...
            textarea.id = 'captchaai-callback-trigger';
...
function executeAutoSubmitRule(rules) {
    for (let i = 0; i < rules.length; i++) {
        ...
        if (regExp.test(location.href)) {
            let textarea = document.createElement('textarea');
            textarea.id = 'captchaai-autosubmit-code';
...
function createSolverButton(captchaType, widgetId, config) {
    let button = $(`
        <div class="captcha-solver captcha-solver_${config.buttonPosition}" data-state="ready" data-captcha-type="${captchaType}" data-widget-id="${widgetId}">
            ...
            <div class="captcha-solver-info">${chrome.i18n.getMessage("solveWithcaptchaai")}</div>
        </div>
    `);
...
function showToast(message) {
    if (!$('body > .captchaai-toast-container').length) {
        $('body').append(`<div class="captchaai-toast-container"></div>`);
    }

    let toastEl = $(`
        <div class="captchaai-toast">
            <img src="${chrome.runtime.getURL("assets/images/logo.svg")}" class="captchaai-toast-logo">
            <button type="button" class="close">&times;</button>
            <div class="captchaai-toast-message">${message}</div>
        </div>
    `).appendTo('.captchaai-toast-container');
