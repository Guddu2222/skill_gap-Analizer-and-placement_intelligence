/* eslint-disable */
export function twitchScript(isBlockerOn) {
  ...
  function doTwitchPlayerTask(isPausePlay, isCheckQuality, isCorrectBuffer, isAutoQuality, setAutoQuality) {
    try {
      ...
      if (!reactRootNode) {
        if (debugMode) console.log('Could not find react root');
        return;
      }
  ...
  function hookFetch() {
    ...
    window.fetch = function (url, init, ...args) {
      if (typeof url === 'string') {
        ...
        if (url.includes('twitch.tv/ads')) {
          if (debugMode) console.log('Block Banner Ads', url);
  ...
  async function start() {
    if (window.Worker.toString().includes('twitch')) {
      if (debugMode) console.log('Code Injected Before true');
    } else {
      if (debugMode) console.log('Code Injected Before false');
