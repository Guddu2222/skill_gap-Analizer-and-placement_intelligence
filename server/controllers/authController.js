...
/**
 * Handles user, student, and college administrator registration.
 */
exports.register = async (req, res) => {
  try {
    ...
    if (role === "student") {
      ...
      if (
        studentCollegeName &&
        typeof studentCollegeName === "string" &&
        studentCollegeName.trim().length >= 3
      ) {
      ...
      const studentPayload = {
        ...
        rollNumber:
          rollNumber && String(rollNumber).trim()
            ...
            : email.split("@")[0],
        collegeRollNumber:
          collegeRollNumber && String(collegeRollNumber).trim()
            ? String(collegeRollNumber).trim()
            : undefined,
        department:
        ...
        degree:
          degree && String(degree).trim() ? String(degree).trim() : undefined,
        ...
        expectedSalaryMin: expectedSalaryMin
          ? parseFloat(expectedSalaryMin)
          : undefined,
        expectedSalaryMax: expectedSalaryMax
          ? parseFloat(expectedSalaryMax)
          : undefined,
        willingToRelocate:
          willingToRelocate !== undefined ? Boolean(willingToRelocate) : true,
      };
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user.isVerified) {
      return res
        .status(400)
        .json({ msg: "Please verify your email first", isVerified: false });
    }
    ...
    // Background process for orphaned student checks
    if (user.role === "college_admin" && user.college) {
      setImmediate(async () => {
        try {
          ...
          const words = canonicalCollege.name
            .split(/\s+/)
            .filter((w) => w.length > 3);
          ...
          const altColleges = await College.find({
            name: nameRegex,
            _id: { $ne: canonicalId },
          })
            .select("_id")
          ...
          if (result.modifiedCount > 0) {
            console.log(
              `[College Repair] Relinked ${result.modifiedCount} student(s) to college ${canonicalId} (${canonicalCollege.name})`,
            );
          }
        } catch (repairErr) {
          console.error(
            "[College Repair] Error during student re-link:",
            repairErr.message,
          );
        }
      });
